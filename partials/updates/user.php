...
if (isset($_POST['submit'])) {
    ...
    $foto = $_POST['urlfoto'];
    $id = $dados['user_id'];
    $crud->updateuser($nome, $email, $telefone, $endereco, $foto, $id);
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    <div class="container mx-auto h-screen flex flex-column justify-center">
        ...
        <form method="POST" 
            class="register p-4 max-w-7xl self-center justify-self-center shadow-lg shadow-gray-400/40 rounded-lg flex flex-col align-between">
            ...
            <div class="nome">
                ...
                <input type="text"
                    class="border border-sky-500 rounded-full h-fit min-w-full focus:outline-none focus:ring focus:ring-sky-500 p-2"
                    id="nome" name="nome" required value="<?php echo "{$dados['nome']}"?>">
            </div>
            <div class="email">
                <label for="email">Email</label>
                <input type="email"
                    class="border border-sky-500 rounded-full h-fit min-w-full focus:outline-none focus:ring focus:ring-sky-500 p-2"
                    id="email" name="email" required value="<?php echo "{$dados['email']}"?>">
            </div>
            <div class="tel">
